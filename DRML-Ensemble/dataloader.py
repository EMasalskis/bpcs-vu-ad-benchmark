...
class DRDataset():
    def __init__(self, dataset_name="Fdataset", drug_neighbor_num=15, disease_neighbor_num=15):
        ...
        self.disease_neighbor_num = disease_neighbor_num
        self.dataset_name = dataset_name
        self.preprocessing()

    def preprocessing(self):
        self.drug_edge = self.build_graph(self.drug_sim, self.drug_neighbor_num)
        self.disease_edge = self.build_graph(self.disease_sim, self.disease_neighbor_num)
        pos_num = self.interactions.sum()
        neg_num = np.prod(self.interactions.shape) - pos_num
        self.pos_weight = neg_num / pos_num
        print(
            f"dataset:{self.dataset_name}, drug:{self.drug_num}, disease:{self.disease_num}, pos weight:{self.pos_weight}")
...
class TargetDRDataset(DRDataset):
    def __init__(self, drug_target_path, disease_target_path, diseaseMapping_path, dataset_name="Fdataset",
                 drug_neighbor_num=15,
                 disease_neighbor_num=15):
        DRDataset.__init__(self, dataset_name=dataset_name, drug_neighbor_num=drug_neighbor_num,
                           disease_neighbor_num=disease_neighbor_num)
        self.drug_target_path = drug_target_path
        self.disease_target_path = disease_target_path
        self.diseaseMapping_path = diseaseMapping_path
        self.drug_target = {}
        self.disease_target = {}
        self.OMIMDisease = {}
        self.ScreenDrug()
    ...
    def ScreenDrug(self):
        DTnetwork = pd.read_csv(self.drug_target_path)
        DTnetwork = DTnetwork.values
        Drugs = set([x[0] for x in DTnetwork])
        for i in DTnetwork:
            if (self.drug_target.keys().__contains__(i[0]) == False):
                self.drug_target[i[0]] = []
            self.drug_target[i[0]].append(i[1])
        c = 0
        delete_drugs = []
        for i in self.drug_name:
            if (i[0] not in Drugs):
                delete_drugs.append(c)
            c += 1

        self.drug_name = np.delete(self.drug_name, delete_drugs, axis=0)
        self.drug_sim = np.delete(self.drug_sim, delete_drugs, axis=1)
        self.drug_sim = np.delete(self.drug_sim, delete_drugs, axis=0)
        self.drug_num = len(self.drug_name)
        self.interactions = np.delete(self.interactions, delete_drugs, axis=0)

    def ScreenDisease(self):
        DGnetwork = pd.read_csv(self.disease_target_path)
        DGnetwork = DGnetwork.values
        Diseases = set([x[1] for x in DGnetwork])
        disease_target = {}
        for i in DGnetwork:
            if (disease_target.keys().__contains__(i[1]) == False):
                disease_target[i[1]] = []
            disease_target[i[1]].append(i[0])

        OMIMtoUMLS = pd.read_csv(self.diseaseMapping_path)
        OMIMtoUMLS = OMIMtoUMLS.values
        self.OMIMDisease = {}

        for x in OMIMtoUMLS:
            D = str(x[0])
            C = x[1]
            if (self.OMIMDisease.keys().__contains__(D) == False):
                self.OMIMDisease[D] = []
            self.OMIMDisease[D].append(C)
        c = 0
        delete_disease = []
        for i in self.disease_name:
            diseaseID = i[0][1:]
            isOK = False
            if (self.OMIMDisease.keys().__contains__(diseaseID)):
                for x in self.OMIMDisease[diseaseID]:
                    if x in Diseases:
                        isOK = True
                        break
            if isOK == False:
                # print(diseaseID)
                delete_disease.append(c)
            c += 1
        self.disease_name = np.delete(self.disease_name, delete_disease, axis=0)
        self.disease_sim = np.delete(self.disease_sim, delete_disease, axis=1)
        self.disease_sim = np.delete(self.disease_sim, delete_disease, axis=0)
        self.interactions = np.delete(self.interactions, delete_disease, axis=1)
        self.disease_num = len(self.disease_name)
        self.disease_target = {}
        for i in self.disease_name:
            self.disease_target[i[0]] = []
            diseaseID = i[0][1:]
            for x in self.OMIMDisease[diseaseID]:
                if (disease_target.keys().__contains__(x)):
                    for y in disease_target[x]:
                        self.disease_target[i[0]].append(y)
